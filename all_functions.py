...
def get_headers(referer):      
    """
    Generates HTTP headers for requests, including cookies, user-agent, and other necessary fields, using the provided referer URL.
    """
    headers = {
        'accept': '*/*',
...
def Get_list_of_projects(country_name, country_short_name):
    """
    Fetches a list of projects for a given country from the World Bank API, transforms the data, and returns the total number of projects and the transformed data.
    """
    headers = get_headers(f'https://data.worldbank.org/country/{country_name}?view=chart')
        
    url = links.get_url('projects', country_short_name)
    response = requests.get(url, headers=headers)
    try:
        dta = response.json()
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON from {url[:10]}: {str(e)}")
        return 0, {}
    ...
    try:
        ...
        data['country_code'] = country_short_name
        transformed_data = transform_projects_data(data)
        return length, transformed_data
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON from {url[:10]}: {str(e)}")
        return length, {}
...
def transform_projects_data(data):
    """
    Transforms the raw data from the World Bank API into a structured format.
    """
    if not data:
        return []
    transformed_data = [] 
    transformed_project = data["list_of_projects"]["projects"]
    transformed_data.append(transformed_project)
    
    return transformed_data

def sectors(sort_name, country_name):
    """
    Fetches and transforms sector data for a given country from the World Bank API.
    """
    ...
    data = response.json()
    data['country_name'] = country_name
    data['country_code'] = sort_name
    transformed_data = transform_sectors_data(data)
    return transformed_data

def transform_sectors_data(data):
    """
    Transforms the raw data from the World Bank API into a structured format.
    """
    if not data:
    ...
    transformed_data = [{'Total Sectors': {'count': value['count'], 'name': value['name']} for key, value in data["sectors"]["facets"]["countryshortname_exact"].items()},
    {'project financial year': {value['name']: value['count'] for key, value in data["sectors"]["facets"]["projectfinancialtype_exact"].items()}},
    {"sectors":{value['name']: value['count'] for key, value in data["sectors"]["facets"]["sector_exact"].items()}},
    {"Sector status":{value['name']: value['count'] for key, value in data["sectors"]["facets"]["status_exact"].items()}},
    {"Theme exact":{value['name']: value['count'] for key, value in data["sectors"]["facets"]["theme_exact"].items()}}
    ]
    return transformed_data

def sectors_information(full_name, sort_name):
    """
    Fetches and transforms sector information for a given country from the World Bank API.
    """
    ...
    data['country_code'] = sort_name
    transformed_data = transform_sectors_information_data(data,sort_name)
    return transformed_data

def extract_value(item):
    """
    Extracts the value from a given item, handling different data types and structures.
    """
    if isinstance(item, dict):
        if 'value' in item:
            return item['value']
        elif '$type' in item and item['$type'] == 'atom':
            return item.get('value', '')
    return item

def clean_dict(d):
    """
    Cleans a dictionary by removing specific keys and handling nested structures.
    """
    if isinstance(d, dict):
        return {k: clean_dict(v) for k, v in d.items() if k != '$type'}
    elif isinstance(d, list):
        return [clean_dict(item) for item in d]
    else:
        return d

def transform_sectors_information_data(data,sort_name):
    """
    Transforms the raw data from the World Bank API into a structured format.
    """
    if not data:
        return []
    sectors_info = [{"Sector Information": data["sectors_information"]["jsonGraph"]["countryMeta"][sort_name]["en"]["value"]},{"Indicators Information": {
    key: clean_dict(extract_value(value))
    for key, value in data["sectors_information"]["jsonGraph"]["indicatorMeta"].items()
        if key != '$type' and not (isinstance(value, dict) and value.get('$type') == 'atom' and 'value' not in value)
    }}]
    return sectors_info

def Indicator_data(sort_name, full_name):
    """
    Fetches and transforms indicator data for a given country from the World Bank API.
    """
    ...
    data['country_name'] = full_name
    data['country_code'] = sort_name
    transformed_data = transform_indicator_data(data,sort_name)
    return transformed_data

def transform_indicator_data(data,sort_name):
    """
    Transforms the raw data from the World Bank API into a structured format.
    """
    if not data:
    ...
    transformed_data = [
        {"Indicator Data":data["indicator"]["jsonGraph"]["indicatorData"][sort_name]}
    ]
    return transformed_data

def indicator_meta_data(sort_name, full_name):  
    """
    Fetches and transforms indicator metadata for a given country from the World Bank API.
    """
    ...
    data['country_name'] = full_name
    data['country_code'] = sort_name
    transformed_data = transform_indicator_meta_data(data)
    return transformed_data

def transform_indicator_meta_data(data):
    """
    Transforms the raw data from the World Bank API into a structured format.
    """
    if not data:
        return []
    meta_data = [{"Indicator Meta Data":data["indicator_meta_data"]["jsonGraph"]["indicatorMeta"]}]
...
def country_indecator_groups_data(full_name):
    """
    Fetches and transforms country indicator groups data for a given country from the World Bank API.
    """
    url = links.get_url('country_indicator_meta_data', "")
    payload = {}
    headers = get_headers(f'https://data.worldbank.org/country/{full_name}?view=chart')
    response = requests.request("GET", url, headers=headers, data=payload)
...
def country_information(sort_name, full_name):
    """
    Fetches and transforms country information for a given country from the World Bank API.
    """
...
def projects_and_operations_data(sort_name, full_name):
    """
    Fetches and transforms projects and operations data for a given country from the World Bank API.
    """
...
def other_indecators_data(sort_name, full_name):
    """
    Fetches and transforms other indicators data for a given country from the World Bank API.
    """
...
def Get_countrys_name_and_short_name():
    """
    Fetches and transforms country names and short names from the World Bank API.
    """
...
def download_files(sort_name):
    """
    Downloads and processes CSV files for a given country from the World Bank API.
    """
...
def project_document_list(sort_code):
    """
    Fetches and transforms project document list for a given country from the World Bank API.
    """
